    ...
    <li>
      <details className="group">
        <summary className="flex cursor-pointer list-none flex-col gap-2 rounded-2xl px-4 py-5 transition-colors hover:bg-ground/50 sm:flex-row sm:items-baseline sm:justify-between sm:gap-8 sm:px-6 [&::-webkit-details-marker]:hidden">
          ...
          <span className="block max-w-[660px]">
            <span className="block text-[17px]">
              {heading} <span className="text-muted">· {subheading}</span>
            </span>
            <span className="mt-1 block text-[15px] leading-relaxed text-muted">{summary}</span>
          </span>
        ...
        <div className="px-4 pb-6 text-[15px] leading-relaxed text-muted sm:px-6">{children}</div>
      </details>
    </li>
  );
}
...
export default function ExperienceList() {
  ...
  return (
    <>
      <ul className="divide-y divide-ink/10 rounded-[26px] bg-shell p-2 sm:p-3">
        {roles.map((role) => (
          <Row
            key={role.company}
          ...
          >
            <ul className="max-w-[720px] list-disc space-y-2 pl-5 marker:text-accent">
        ...
        >
          <p className="max-w-[720px]">Coursework: {education.coursework.join(', ')}.</p>
        </Row>
      </ul>

      <dl className="mt-[18px] grid gap-x-8 gap-y-3 rounded-[26px] bg-shell px-6 py-7 text-[15px] leading-relaxed sm:grid-cols-[8rem_1fr] sm:px-9">
